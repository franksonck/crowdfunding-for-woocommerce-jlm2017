...
class Alg_Crowdfunding_Product_Open_Pricing {
		...
		if ( $this->is_open_price_product( $the_product ) ) {
			...
			if ( $min_price > 0 ) {
				if ( ! isset( $_POST['alg_crowdfunding_open_price'] ) || '' === $_POST['alg_crowdfunding_open_price'] ) {
					wc_add_notice( get_option( 'alg_crowdfunding_product_open_price_messages_required', __( 'Price is required!', 'alg-woocommerce-crowdfunding' ) ), 'error' );
					return false;
				}
				if ( $_POST['alg_crowdfunding_open_price'] < $min_price ) {
					wc_add_notice( get_option( 'alg_crowdfunding_product_open_price_messages_to_small', __( 'Entered price is to small!', 'alg-woocommerce-crowdfunding' ) ), 'error' );
					return false;
				}
			}
			if ( $max_price > 0 ) {
				if ( isset( $_POST['alg_crowdfunding_open_price'] ) && $_POST['alg_crowdfunding_open_price'] > $max_price ) {
					wc_add_notice( get_option( 'alg_crowdfunding_product_open_price_messages_to_big', __( 'Entered price is to big!', 'alg-woocommerce-crowdfunding' ) ), 'error' );
					return false;
				}
			}
		}
	...
	function add_open_price_input_field_to_frontend() {
		...
		if ( $this->is_open_price_product( $the_product ) ) {
			$title = get_option( 'alg_crowdfunding_product_open_price_label_frontend', __( 'Name Your Price', 'alg-woocommerce-crowdfunding' ) );
